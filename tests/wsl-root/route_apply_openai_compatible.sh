...
cat > "$env_file" <<'ENV'
OPENAI_RELAY_API_KEY=relay-secret-token
...
output="$(
  ...
  "$sandbox_bin/hermes-control-route-apply.sh" \
    external.openai-relay openai-compatible https://openai-relay.example.com/v1 gpt-4.1 OPENAI_RELAY_API_KEY
)"
...
assert payload["provider_kind"] == "openai-compatible", payload
assert payload["secret_env_key"] == "OPENAI_RELAY_API_KEY", payload
...
assert env_values["OPENAI_RELAY_API_KEY"] == "relay-secret-token", env_values
assert env_values["LM_API_KEY"] == "relay-secret-token", env_values
assert env_values["LM_BASE_URL"] == "https://openai-relay.example.com/v1", env_values
assert env_values["LM_MODEL"] == "gpt-4.1", env_values
